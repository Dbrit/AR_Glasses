...
void ST7735_OutChar(char ch){
  if((ch == 10) || (ch == 13) || (ch == 27)){
    ...
    ST7735_DrawStringBG(0,StY,"                     ",StTextColor, BgTextColor);
    return;
  }
...
// ************** ST7735_SetBGColor ************************
// Sets the color in which the BG will be printed
// Input:  16-bit packed color
// Output: none
// ********************************************************
void ST7735_SetBGColor(uint16_t color) {
	BgTextColor = color;
}
...
void ST7735_XYplotInit(char *title, int32_t minX, int32_t maxX, 
  int32_t minY, int32_t maxY, uint16_t bcolor) {
	...
	// x goes from [0, 127]
	// y goes from [32, 159]
	ST7735_FillRect(0,32,128-0, 160-32, bcolor); // top 32 rows reserved for title
}
...
void ST7735_XYplot(uint32_t num, int32_t bufX[], 
  int32_t bufY[], uint16_t color) {
	while (num--) {
		int32_t x = (127 * (*bufX - curPlot.minX)) / (curPlot.maxX - curPlot.minX);
		int32_t y = (127 * (curPlot.maxY - *bufY)) / (curPlot.maxY - curPlot.minY) + 32;
